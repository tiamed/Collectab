...
export default function OrgSettingsModal({ org, onSave, onClose }: OrgSettingsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSave({
        ...
        icon: icon !== org.icon ? icon : undefined,
      });
      onClose();
    } finally {
      setSaving(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--backdrop)]" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[var(--foreground)]">Organization Settings</h3>
